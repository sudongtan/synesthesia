...
images_all = [os.path.join(image_dir, name) for name in os.listdir(image_dir)]
images = images_all # name for name in images_all if name.split(".")[-1] == "jpg"]
#print(images_all)
#print(images)
# for image in images:
#     print(len(Image.open(image).size))
images = [image for image in images if Image.open(image).mode == "RGB"]
names = [os.path.basename(name).split(".")[0] for name in images]
print("Number of images: ", len(images))
names2 = names[101:109]
...
for  i, (name, image) in enumerate(zip(names, images)):
    ...
    with torch.no_grad():
        ...
        for i, (output, n) in enumerate(zip(outputs, names2)):
            name = f'{n}-{i}.mid'
